// pages/index.vue
<template>
  <c-box as="main" d="flex" align-items="space-between" flex-direction="column" w="auto" p="16">
  ...
  computed: {
    filteredComponents() {
      return chakraComponents.filter(component => {
        return this.lowercase(component.name).includes(
          this.lowercase(this.search)
        );
      });
    }
  },
